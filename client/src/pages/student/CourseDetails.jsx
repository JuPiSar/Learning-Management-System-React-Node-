...
const CourseDetails = () => {
  ...
  const [playerDate, setPlayerData] = useState(null);
  ...
  <>
    <div className='flex flex-col-reverse md:flex-row gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
      ...
      <div className='max-w-xl z-10 text-gray-500'>
        ...
        <p className='text-sm'>Course by <span className='text-blue-600 underline'>JUPISAR</span></p>
            ...
            <div key={i} className='border border-gray-300 bg-white mb-2 rounded'>
              <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={() => toggleSection(i)}>
                <div className='flex items-center gap-2'>
                  <img className={`transform transition-transform ${openSections[i] ? 'rotate-180' : ''}`} src={assets.down_arrow_icon} alt="down arrow" />
              ...
              <div className={`overflow-hidden transition-all duration-300 ${openSections[i] ? "max-h-96" : "max-h-0"}`}>
                <ul className='list-disc md:pl-10 pl-4 pr-4 py-2 text-gray-600 border-t border-gray-300'>
                  {chapter.chapterContent.map((lect, i) => (
                    <li className='flex items-start gap-2 py-1' key={i}>
                      ...
                      <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-default'>
                        <p>{lect.lectureTitle}</p>
                        <div className='flex gap-2'>{lect.isPreviewFree && 
                          <p onClick={() => setPlayerData({
                            videoId: lect.lectureUrl.split('/').pop()
                          })} className='text-blue-500 cursor-pointer'>Preview</p>}
                          <p>{humanizeDuration(lect.lectureDuration *60 * 1000, {units: ['h', 'm']})}</p>
                        </div>
      ...
      <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
        {
          playerDate? 
            <YouTube videoId={playerDate.videoId} opts={{playerVars: {
              autoplay: 1
            }}} iframeClassName='w-full aspect-video'/>
          : <img src={courseData.courseThumbnail} alt="course_thubnail" />
        }
